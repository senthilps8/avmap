# Taken from https://github.com/facebookresearch/habitat-lab
import numba
import numpy as np
...
def _draw_loop(
    ...
    for angle in angles:
        draw_fog_of_war_line(
            ...
            fog_of_war_mask,
            current_point,
            current_point + max_line_len * np.array(
                [np.cos(current_angle + angle),
                 np.sin(current_angle + angle)]),
        )
...
def reveal_fog_of_war(
    ...
    angles = np.arange(-fov / 2,
                       fov / 2,
                       step=1.0 / max_line_len,
                       dtype=np.float32)
